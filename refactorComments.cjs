...
const commentsContent = `import { useState } from 'react';
...
fs.writeFileSync('src/components/Comments.tsx', commentsContent);
...
const stateTarget = `  const [commentText, setCommentText] = useState("");
  const [commentName, setCommentName] = useState("");
  const [commentEmail, setCommentEmail] = useState("");
  const [commentsList, setCommentsList] = useState([
    {
...
articleContent = articleContent.replace(stateTarget, "");

// Remove handler
const handlerTarget = `  const handleSubmitComment = (e: React.FormEvent) => {
    e.preventDefault();
    if (!commentText.trim() || !commentName.trim()) return;
...
articleContent = articleContent.replace(handlerTarget, "");

// Replace JSX
const jsxTargetStart = `{/* 11. Comments Section */}`;
const jsxTargetEnd = `{/* 12. Sidebar Content (Desktop) */}`;
const startIndex = articleContent.indexOf(jsxTargetStart);
const endIndex = articleContent.indexOf(jsxTargetEnd);

if (startIndex !== -1 && endIndex !== -1) {
  articleContent = articleContent.substring(0, startIndex) +
    "{/* 11. Comments Section */}\n            <Comments articleId={article.id} />\n\n          </div>\n\n          " +
    articleContent.substring(endIndex);
}
