...
export function Article() {
  ...
  const handleSubmitComment = (e: React.FormEvent) => {
    ...
    const newComment = {
      ...
      author: commentName,
      role: "Reader",
      avatar: \`https://api.dicebear.com/7.x/initials/svg?seed=\${commentName}\`,
      text: commentText,
              ...
              <div className="space-y-6">
                {commentsList.map(comment => (
                  <div key={comment.id} className="flex gap-4">
                    <img src={comment.avatar} alt={comment.author} className="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 shrink-0" />
                    <div>
